...
import { renderInvestigationReport, renderLeanReport, verifyInvestigationReport } from "./report.ts";
...
const leanResult: LeanReportResult = {
  schemaVersion: 2,
  run: {
    id: "run-lean-report",
    status: "COMPLETED",
  ...
  findings: [
    {
      ...
      order: 2,
      section: "Education",
      claim: "Reported education",
      anchor: { kind: "PDF_TEXT", page: 2, lineStart: 1, lineEnd: 1, exact: "Education" },
      evidence: "No sufficiently specific public record was found.",
      notes: "The absence of public evidence is not evidence that the claim is false.",
      status: 0,
      sources: [],
    },
    {
      ...
      claim: "CE-SW Runtimes, Arm Ltd., Cambridge, UK 2023–present — Principal Software Engineer",
      anchor: { kind: "PDF_TEXT", page: 1, lineStart: 12, lineEnd: 13, exact: "CE-SW Runtimes, Arm Ltd., Cambridge, UK 2023–present\nPrincipal Software Engineer" },
      evidence: "The public record consistently identifies the role and interval.",
      notes: "",
      status: 2,
      sources: [{ sourceRef: "S1", title: "Public profile", url: "https://example.com/profile" }],
    },
  ],
};

test("renders the lean report deterministically in résumé order", async () => {
  const first = await renderLeanReport(leanResult);
  const second = await renderLeanReport(structuredClone(leanResult));
  ...
  assert.doesNotMatch(text, /authority|facet|critic|adjudicat/i);
});
