...
function latestUserText(body: Record<string, unknown>): string {
  ...
  return user.content.flatMap((part) => part && typeof part === "object" && typeof (part as { text?: unknown }).text === "string" ? [(part as { text: string }).text] : []).join("\n");
}
...
export function createHeadlessFixtureCompletion(): (body: Record<string, unknown>, agent: string) => Promise<Completion> {
  ...
  return async (body, agent) => {
    ...
    if (agent === "lead-researcher") {
      if (/research is complete|publishing-only recovery/i.test(latestUserText(body))) {
        publishingCalls += 1;
