...
export async function renderLeanReport(value: LeanReportResult): Promise<Buffer> {
  ...
  const subheading = (title: string) => document.moveDown(0.25).font("Helvetica-Bold").fontSize(10.5).fillColor("#14213D").text(title).moveDown(0.15);
  const paragraph = (text: string) => document.font("Helvetica").fontSize(9.3).fillColor("#273449").text(text, { lineGap: 2 }).moveDown(0.4);
