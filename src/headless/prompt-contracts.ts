...
Process only the supplied unresolved résumé line window; preceding context lines are read-only. Return at most five claims. Every supplied line must appear exactly once in a claim facet, an exclusion, or deferredLineIds. The earliest unresolved line must not be deferred. Use the exact supplied line IDs; never return page numbers, canonical IDs, verdicts, strengths, or evidence.

Every facet is one independently adjudicable predicate and must include: key, kind, label, sourceFragment, materiality, and lineIds. kind is exactly one of IDENTITY, ORGANIZATION, ORG_UNIT, TITLE, INTERVAL, LOCATION, ACTIVITY, RESPONSIBILITY, CONTRIBUTION, OUTPUT, EDUCATION, AFFILIATION, or OTHER. sourceFragment is the smallest byte-exact substring of its assigned submission lines that expresses the facet. Employer, unit, each title, interval, location, activity, responsibility, contribution, and output are separate facets even when printed on one line. Only INTERVAL may include normalized from or to values. Preserve uncertain and low-materiality facts as claims rather than silently excluding them.`;

export const EVIDENCE_JUDGE_PROMPT_CONTRACT = `MODE: EVIDENCE_JUDGE

Judge exactly the supplied frozen claim bundle. For every facet, return exactly one judgment for every assigned candidate excerpt. The only relations are SUPPORTS, CONTRADICTS, CONTEXT, and IRRELEVANT. A candidate about the same person, employer, project, or technology is IRRELEVANT unless its exact text establishes or contradicts that specific facet. Relevant background that does not prove or disprove the candidate-specific facet is CONTEXT. CONTEXT and DISCOVERY_ONLY sources can never SUPPORT or CONTRADICT. CONTRADICTS requires explicit negation or incompatible facts in the same or overlapping temporal scope; an undated, current, earlier, or later alternative title is IRRELEVANT to a dated title unless the periods conflict. Return only bundleId, claimKey, facetKey, excerptRef, relation, and a concise reason; never return quotes, source refs, paths, URLs, authority, durable evidence IDs, verdicts, or strengths. Do not call tools or research.`;
