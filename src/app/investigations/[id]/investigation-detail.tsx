...
export function InvestigationDetail({ initial }: { initial: Detail }) {
  ...
  return <>
    <section className="case-hero"><div><div className="case-kicker"><StatusBadge status={detail.status} /><span>Case {shortId(detail.id)}</span><span>{display(detail.runtimeKind)}</span></div><h1>{caseTitle(detail)}</h1><p className="lede">Evidence-focused review · synthetic development data</p></div><div className="hero-actions"><a className="button button-secondary" href={`/api/investigations/${detail.id}/report.pdf`}>Download report</a>{!terminal.has(detail.status) ? <button className="button button-danger" type="button" onClick={cancel} disabled={cancelPending}>{cancelPending ? "Cancelling…" : "Cancel run"}</button> : null}</div></section>
    ...
    <section className="tab-panel" role="tabpanel">
      ...
      {tab === "Agent trace" ? <div className="trace-layout"><aside className="trace-summary"><p className="panel-label">Current operation</p><h2>{detail.events.at(-1) ? display(detail.events.at(-1)?.status) : display(detail.status)}</h2><p>{detail.events.at(-1)?.publicRationale ? display(detail.events.at(-1)?.publicRationale) : "Operational events appear here without private model reasoning."}</p><dl><dt>Model budget</dt><dd>${Number((detail.budgetCounters as Row | undefined)?.modelUsd ?? 0).toFixed(3)} / $5</dd><dt>Provider budget</dt><dd>${Number((detail.budgetCounters as Row | undefined)?.providerUsd ?? 0).toFixed(3)} / $10</dd><dt>Manifest</dt><dd><code>{String(detail.runtimeManifestHash ?? "pending").slice(0, 12)}</code></dd></dl></aside><div className="trace-stream">{detail.events.map((event) => <article className="trace-event" key={String(event.id)}><time>{formatDate(event.createdAt)}</time><span className="trace-agent">{display(event.agent)}</span><div><strong>{display(event.eventType)}</strong>{event.tool ? <code>{display(event.tool)}</code> : null}<p>{event.publicRationale ? display(event.publicRationale) : `${display(event.status)}${event.source ? ` · ${display(event.source)}` : ""}`}</p></div></article>)}{detail.events.length === 0 ? <Empty label="Waiting for the runner to claim this case." /> : null}</div></div> : null}
    </section>
  </>;
}
