...
export async function fixtureCompletion(body: Record<string, unknown>, investigationId: string, runId: string): Promise<Completion> {
  ...
  if (names.has("claim.create") && !isStructured && !isProfessionalFixture) {
    ...
    if ((runs[0]?.researchWaveCount ?? 0) === 0) return { toolCall: { name: "research.begin_wave", arguments: { waveKind: "INITIAL", questionIds: [question.id], publicRationale: "Beginning the single broad research wave for the active synthetic chronology question." } } };
  ...
  if (isProfessionalFixture) {
    ...
    if (!evidence.length) return { toolCall: { name: "evidence.capture", arguments: { artifactId: citableArtifact.id, exactQuote: "Synthetic Candidate held the title Principal Engineer at Acme Synthetic Labs from 2021 through 2025.", sourceLocation: { jsonPath: "$.records[0].text" }, relation: "SUPPORTS", claimIds: [claim.id], facetKeys: ["employer", "title", "tenure"], entityIds: [entity.id] } } };
  ...
  const summary = { professionalIdentity: { status: "AMBIGUOUS", summary: "The synthetic root person comes from intake; no external account was linked because the two-anchor threshold was not met.", evidenceIds: [] }, professionalTimelineSummary: observations.length ? "One synthetic employment observation records the Principal Engineer title at Acme Synthetic Labs for 2021–2025." : "No professional timeline observations were saved.", professionalTimelineEvidenceIds: observations.length ? evidence.map(({ id }) => id).slice(0, 3) : [], professionalIdentityClaimIds: [], professionalTimelineClaimIds: observations.length && claims[0] ? [claims[0].id] : [], strongestEvidenceIds: evidence.map(({ id }) => id).slice(0, 3), strongestEvidenceByClaim: observations.length && claims[0] ? [{ claimId: claims[0].id, facetKeys: claims[0].facets.map(({ key }) => key), evidenceIds: evidence.filter((item) => item.claimIds.includes(claims[0]!.id)).map(({ id }) => id).slice(0, 3) }] : [], materialInconsistencies: [], unresolvedMaterialClaimIds: claims.filter((claim) => !evidence.some((item) => item.claimIds.includes(claim.id))).map(({ id }) => id), investigationLimitations: ["All sources and identities in this run are deterministic synthetic fixtures."] };
