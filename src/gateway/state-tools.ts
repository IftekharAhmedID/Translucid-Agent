...
export const stateToolNames = [
  ...
  "artifact.excerpts", "artifact.lookup",
...
const schemas: Record<StateToolName, z.ZodType> = {
  ...
  "artifact.lookup": z.object({ questionIds: z.array(uuid).min(1).max(12).optional(), sourceUrl: z.url().optional(), providerRoute: z.string().min(1).max(200).optional(), kind: z.string().min(1).max(100).optional() }).strict().refine((value) => Boolean(value.questionIds?.length || value.sourceUrl || value.providerRoute || value.kind), "At least one artifact lookup filter is required."),
  ...
  "research.authorize_task": z.object({ assignmentId: uuid, role: z.enum(["professional-investigator", "github-investigator", "web-records-investigator", "social-investigator"]), questionIds: z.array(uuid).min(1).max(3) }).strict(),
  "research.complete_task": z.object({ assignmentId: uuid, role: z.enum(["professional-investigator", "github-investigator", "web-records-investigator", "social-investigator"]) }).strict(),
  "evidence.capture": z.object({ artifactId: uuid, exactQuote: z.string().min(1).max(12_000), sourceLocation: z.record(z.string(), z.unknown()).optional(), relation: z.enum(["SUPPORTS", "CONTRADICTS", "CONTEXT"]), claimIds: z.array(uuid).max(100), facetKeys: z.array(z.string().regex(/^[a-z][a-z0-9_]{0,63}$/)).max(12), entityIds: z.array(uuid).max(100) }).strict().superRefine((value, context) => { if (value.relation !== "CONTEXT" && value.claimIds.length !== 1) context.addIssue({ code: "custom", message: `${value.relation} evidence must reference exactly one claim.` }); if (value.relation !== "CONTEXT" && value.facetKeys.length < 1) context.addIssue({ code: "custom", message: `${value.relation} evidence must reference at least one declared claim facet.` }); if (value.relation === "CONTEXT" && value.facetKeys.length > 0 && value.claimIds.length === 0) context.addIssue({ code: "custom", message: "Context evidence facet keys require at least one referenced claim." }); }),
...
export async function executeStateTool(name: StateToolName, raw: unknown, context: OperationalContext): Promise<unknown> {
  ...
  switch (name) {
    ...
    case "research.resolve": {
      ...
      return resolveResearchQuestion({ ...base, questionId: String(args.questionId), selectedRoute: question?.selectedRoute ?? "NO_ROUTE", status: args.status as "RESOLVED" | "EXHAUSTED" | "SKIPPED", resolutionSummary: String(args.resolutionSummary), reviewedArtifactIds: args.reviewedArtifactIds as string[] | undefined });
    }
    case "research.list":
    ...
    case "artifact.lookup":
      return lookupArtifacts(context.investigationId, context.runId, { questionIds: args.questionIds as string[] | undefined, sourceUrl: args.sourceUrl as string | undefined, providerRoute: args.providerRoute as string | undefined, kind: args.kind as string | undefined });
    ...
    case "research.authorize_task":
      return authorizeResearchTask({ ...base, assignmentId: String(args.assignmentId), role: args.role as "professional-investigator" | "github-investigator" | "web-records-investigator" | "social-investigator", questionIds: args.questionIds as string[], agent: context.agent, sessionId: context.sessionId });
    case "research.complete_task":
      return completeResearchTask({ ...base, assignmentId: String(args.assignmentId), role: args.role as "professional-investigator" | "github-investigator" | "web-records-investigator" | "social-investigator", agent: context.agent, sessionId: context.sessionId });
