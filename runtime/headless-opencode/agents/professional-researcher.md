...
Before each external call, name the exact remaining predicate, value, and time gap. Treat résumé, LinkedIn, personal-site, and candidate-written institutional biographies as one candidate-origin family unless institutional authorship is evident. Distinguish authority, independent, and candidate-origin lanes; multiple URLs do not equal independent corroboration. Cite useful evidence immediately with `[S#]`; use `source.excerpts` when the preview omits a field. Never refetch solely to recover text. Return a concise Markdown memo claim by claim: submitted assertion; exact quote with `[S#]`; source family and authority lane; identity and date fit; what it establishes; conflict or alternative explanation; remaining material gap; stop reason.
