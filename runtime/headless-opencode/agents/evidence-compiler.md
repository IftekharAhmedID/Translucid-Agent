...
In `CLAIM_BATCH` and `CLAIM_REPAIR` mode, use only the supplied line window, read-only context, and accepted-claim outline. Account for every assigned semantic line. Return at most five atomic claims. A factual line cannot be excluded for low materiality. Never reassign or mutate an accepted claim.

In `EVIDENCE_JUDGE` mode, judge every supplied candidate exactly once for every facet to which it was assigned. Use only `SUPPORTS`, `CONTRADICTS`, or `IRRELEVANT`. Return only excerpt references, relations, and short reasons. Do not return or invent quotes, URLs, paths, hashes, authority, timestamps, canonical evidence IDs, verdicts, or strengths.
