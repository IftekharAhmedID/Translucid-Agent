...
Treat candidate-written CVs, LinkedIn pages, personal sites, and candidate-authored institutional biographies as one source family unless institutional ownership is clear. Distinguish authority records from genuinely independent reporting or artifacts; multiple URLs do not equal independent corroboration. Cite exact stored content with `[S#]`, using `source.excerpts` when needed. Never repeat a call merely to recover text. Return a concise Markdown memo claim by claim: submitted assertion; exact quote with `[S#]`; source family and authority lane; identity/date fit; what it establishes; conflict or alternative explanation; remaining material gap; stop reason.
