...
const plugin: Plugin = async () => ({
  ...
  tool: {
    ...
    "professional.profile": gatewayTool("professional.profile", "Retrieve professional profile data with code-enforced LinkdAPI then conditional Bright Data routing.", { ...common, username: z.string().min(2).max(200), requiredMaterialField: z.string().max(100).optional() }),
    ...
    "entity.upsert": gatewayTool("entity.upsert", "Create or update a distinct entity without assuming identity.", { type: z.enum(["PERSON", "ORGANIZATION", "ACCOUNT", "WEBSITE", "PUBLICATION", "PATENT", "PACKAGE"]), canonicalName: z.string().min(1).max(500), metadata: z.record(z.string(), z.any()).optional() }),
    ...
    "entity.link": gatewayTool("entity.link", "Link entities only with at least two independent evidence-backed anchors.", { fromEntityId: z.string().uuid(), toEntityId: z.string().uuid(), relationship: z.string().min(1).max(100), anchors: z.array(z.object({ type: z.enum(["EMPLOYER_OVERLAP", "VERIFIED_DOMAIN", "CROSS_LINKED_ACCOUNT", "LOCATION_HISTORY", "REPOSITORY_IDENTITY", "AUTHORED_PAGE"]), evidenceId: z.string().uuid(), sourceKey: z.string().min(1) })).min(2).max(20) }),
