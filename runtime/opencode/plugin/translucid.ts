...
const plugin: Plugin = async ({ client }) => ({
  ...
  tool: {
    ...
    "evidence.capture": gatewayTool("evidence.capture", "Create one evidence edge from an exact quote in an immutable non-snippet artifact. SUPPORTS and CONTRADICTS each require exactly one claim; CONTEXT may have zero or more claim associations and is never citation-eligible. Source authority is assigned by the gateway.", { artifactId: z.string().uuid(), exactQuote: z.string().min(1).max(12000), sourceLocation: z.record(z.string(), z.any()).optional(), relation: z.enum(["SUPPORTS", "CONTRADICTS", "CONTEXT"]), claimIds: z.array(z.string().uuid()).max(100), entityIds: z.array(z.string().uuid()).max(100) }),
