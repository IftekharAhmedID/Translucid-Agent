...
import { isSparseDocument, isSparsePage, MAX_PDF_PAGES } from "../src/core/input.ts";
...
async function extractPdf(pdfPath: string) {
  ...
  const pages: Array<{ page: number; text: string; sparse: boolean; imagePath?: string }> = [];
  for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
    const page = await document.getPage(pageNumber);
    const content = await page.getTextContent();
    const text = content.items.map((item) => "str" in item ? item.str : "").join(" ").replace(/\s+/g, " ").trim();
    pages.push({ page: pageNumber, text, sparse: isSparsePage(text) });
  }
  ...
  const pagesPath = join(inputDirectory, "resume.pages.json");
  await writeFile(cleanTextPath, pages.map((page) => `--- Page ${page.page} ---\n${page.text}`).join("\n\n"));
  await writeFile(pagesPath, JSON.stringify(pages, null, 2));
  return { originalPath: pdfPath, cleanTextPath, pagesPath, pageCount: pages.length, sparsePageNumbers: pages.filter((page) => page.sparse).map((page) => page.page) };
}

async function main() {
  await mkdir(inputDirectory, { recursive: true });
  const manifestPath = join(inputDirectory, "manifest.json");
  const seed = JSON.parse(await readFile(join(inputDirectory, "intake.json"), "utf8")) as Record<string, unknown>;
  let pdf: unknown;
  if (typeof seed.pdfPath === "string") pdf = await extractPdf(seed.pdfPath);
  await writeFile(manifestPath, JSON.stringify({ ...seed, pdf, generatedAt: new Date().toISOString() }, null, 2));
}
